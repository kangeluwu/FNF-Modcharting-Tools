...
class Modifier
{
    ...
    public function checkLane(lane:Int):Bool //returns true if should display on current lane
    {
        ...
        switch(type)
        {
            ...
            case OPPONENTONLY:
										#if RCE
									return stupidStrum == PlayState.instance.currentOpponentStrum;
										#else
                return lane < NoteMovement.keyCount;
										#end
...
class ArrowPath extends Modifier {
	...
	if (Paths.fileExists("data/"+PlayState.SONG.song.toLowerCase()+"/customMods/path.txt", TEXT))
        ...
        #end
        {
            var newPosition = executePath(0, curPos,lane, #if RCE Math.floor(lane/4) #else lane < 4 ? 0 : 1#end ,new Vector4(noteData.x, noteData.y, noteData.z, 0));
